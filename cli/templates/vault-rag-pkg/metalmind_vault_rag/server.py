...
def related_notes(file: str) -> dict:
    ...
    forward_stems = _parse_links(text)
    forward = [
    ...
    for stem, p in index.items():
        if stem == target_stem:
            continue
        if target_stem in _parse_links(p.read_text(encoding="utf-8", errors="ignore")):
...
def expand_search(query: str, k: int = 5) -> dict:
    ...
    for h in hits:
        ...
        if not path.exists():
            continue
        links = _parse_links(path.read_text(encoding="utf-8", errors="ignore"))
        resolved = [
            {"stem": s, "path": str(index[s].relative_to(VAULT))}
            for s in links
            if s in index
