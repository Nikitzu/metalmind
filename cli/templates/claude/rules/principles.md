...
- **End-of-session memory**: Use `/save` to persist key decisions, discoveries, and context into the Obsidian vault at `~/Knowledge/`. Recall via `Bash: {{RECALL_CMD}} "<query>"` (metalmind CLI — no MCP token cost). Do not write to legacy `MEMORY.md` files — that system is retired.
