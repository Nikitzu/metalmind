# claude-knowledge-stack aliases
# Source this from ~/.zshrc:  source ~/.claude-knowledge-stack/aliases.sh
# VAULT_PATH is exported so the Python code picks it up.

export VAULT_PATH="${VAULT_PATH:-$HOME/Knowledge}"
VAULT_STACK="$VAULT_PATH/.claude-stack"
...
alias vault-watcher-start='launchctl load ~/Library/LaunchAgents/com.claude.vault-indexer.plist'
alias vault-watcher-stop='launchctl unload ~/Library/LaunchAgents/com.claude.vault-indexer.plist'
