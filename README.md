...
- **Session-start awareness without nagging.** metalmind installs a Claude Code SessionStart hook plus a top-of-file block in `~/.claude/CLAUDE.md` with explicit WHEN→DO triggers, so every new Claude session discovers the vault on its own - no "did you check memory?" prompting. After an upgrade the next metalmind command re-stamps once on its own; `metalmind burn brass` (alias: `stamp`) re-stamps anytime.

- **Vault writes without drift.** `metalmind scribe <create|update|patch|supersede|delete|archive|list|show|rename|backfill-type>` (alias: `note`) is the CRUD interface agents use *instead of* raw `Write`. It stamps frontmatter (`kind:` plus a matching `type:`, so the vault opens typed in both Obsidian and Tolaria), picks the right folder (`Plans/ Learnings/ Work/ Work/MOCs/ Daily/ Inbox/ Memory/ Personal/ Archive/`), auto-links the project MOC, and on `rename` rewrites `[[wikilinks]]` across the vault. `supersede <old> <new>` marks a decision replaced by its successor - recall downweights the old note and every hit from it carries `superseded_by` so agents land on current truth. Notes can also carry `code: ["repo#symbol"]` refs (stamped via `--code`); `tap copper --verify-code` and `doctor` flag refs whose code no longer exists in the forge-registered repos, so a note's claims stay checkable against the code they describe. `metalmind ingest auto-memory` imports Claude Code's native auto-memory topic files as `Memory/` notes - idempotent, hash-guarded, and conflict-safe - so native memory feeds the vault instead of competing with it. With the judge enabled, `create` and `update` refuse a draft an existing note already covers, name that note, and suggest `scribe update`; `--force` overrides. Every judged call is logged without content to `~/.metalmind/judge-log.jsonl`; `metalmind judge report` turns a week of use into refusal, override and confidence numbers; with `judge.logContent` on, `judge review` lets you label decisions and the report becomes precision per band. `patch --frontmatter` applies `--find/--replace` to the frontmatter instead of the body and refuses a result that is not valid YAML, which is how a note with an unquoted `title: a: b` gets repaired; `backfill-type` adds `type:` to older notes that only carry `kind:`. Body on stdin; every mutating verb supports `--dry-run`. Daily notes for non-today dates require `--date <YYYY-MM-DD>` to acknowledge the target explicitly.
...
The `+rerank` columns describe the cross-encoder tier that shipped through v0.24.1; it is replaced by the opt-in Jev judge, measured privately in `bench/judge-relevance/`. Remeasured for v0.23.0, which cut reranking from 7.5s to 1.3s per query. This
...
Published at [npmjs.com/package/metalmind](https://www.npmjs.com/package/metalmind) · current release `v0.29.0`.
...
| `metalmind scribe <verb>` | `metalmind note <verb>` | Vault CRUD: `create \| update \| patch \| supersede \| delete \| archive \| rename \| list \| show \| backfill-type` |
...
| Vault | Plain markdown at `~/Knowledge/` ([Obsidian](https://obsidian.md)- and [Tolaria](https://tolaria.md)-compatible, neither required) |
